  ...
  const PointCloud & source_cloud_body) const
{
  ...
  const double epsilon = config_.source_covariance_plane_epsilon;

  // 近傍が足りない点の既定値: 等方 (epsilon * I)。 重みをほぼ効かせない。
  std::vector<Eigen::Matrix3d> source_covariances(
    num_points, Eigen::Matrix3d::Identity() * epsilon);

  // 近傍点数 k は最低 3 点ないと平面方向 2 + 法線 1 を分離できない。
  const int num_neighbors = config_.source_covariance_num_neighbors;
  ...
  for (std::size_t pcl_index = 0; pcl_index < pcl_cloud->points.size(); ++pcl_index) {
    ...
    Eigen::Vector3d regularized_eigenvalues(epsilon, 1.0, 1.0);
