...
//
// pylot_lio のメインノード。1 つのノードで以下を行う:
//   1) ROS パラメータから 4 つの抽象実装 (preprocessor / map / registration / estimator) を生成
//   2) /livox/imu を購読して state_estimator.predictWithImu に渡す
//   3) /livox/lidar を購読して preprocessor → state_estimator.updateWithScan に渡す
//   4) /lio/odom (nav_msgs/Odometry) と /lio/cloud_world (sensor_msgs/PointCloud2) を publish
//   5) /lio/diag (std_msgs/String, JSON) で 1 スキャン分の診断値を流す
//
// 起動時に 1 回パラメータを読むだけで、ランタイムでのバックエンド切り替えはしない。
// 切り替えるときはノードを再起動する。
